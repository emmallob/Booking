...
class Api {
        ...
        $this->endpoints = [
            ...
            "reservations" => [
                ...
                "GET" => [
                    "reserve" => [
                        "params" => [
                            "ticket_serial_id" => "required - The serial number for the ticket (autogenerated for Free Events)",
    ...
    */
    ...
    final function apiHandler(array $params, $endPoints) {
        ...
        // check if the users endpoint was parsed
        elseif( in_array($this->inner_url, ["events", "halls", "tickets", "reservations"]) ) {
        ...
        elseif( $this->inner_url == "remove" ) {
            ...
            if(($params->item == "hall") && !$this->accessCheck->hasAccess("delete", "halls")) {
                // permission denied message
                $result['result'] = self::PERMISSION_DENIED;
            } elseif(($params->item == "user") && !$this->accessCheck->hasAccess("delete", "users")) {
                // permission denied message
                $result['result'] = self::PERMISSION_DENIED;
            } elseif(($params->item == "event") && !$this->accessCheck->hasAccess("delete", "events")) {
                // permission denied message
                $result['result'] = self::PERMISSION_DENIED;
            } elseif(($params->item == "ticket") && !$this->accessCheck->hasAccess("delete", "tickets")) {
                // permission denied message
                $result['result'] = self::PERMISSION_DENIED;
            } elseif(($params->item == "reservation") && !$this->accessCheck->hasAccess("delete", "reservations")) {
                // permission denied message
                $result['result'] = self::PERMISSION_DENIED;
            } else {
