...
class Api {
    ...
    public function __construct(array $param) {
         ...
         * global variables
         **/
        global $session, $accessObject, $booking, $usersClass, $bookingClass, $medics;
    ...
    */
    ...
    final function requestHandler(stdClass $params) {
        ...
        $params->user_guid = $this->userId;
        $params->client_guid = $this->clientId;
        

        $params->userData = (object) $this->userData;

        // set the default limit to 1000
        $params->limit = isset($params->limit) ? (int) $params->limit : $this->bookingClass->global_limit;
        ...
        if(method_exists($classObject, $this->outer_url)) {
            ...
            $request = (array) $classObject->$method($params);
            // set the response code to return
            $code = (isset($request['code'])) ? $request['code'] : 200;
            // set the result
            $result['result'] =  $request["msg"] ?? $request;
            ...
            // if remote_request parameter was parsed
            if(isset($request['remote_request'])) {
                // set the remote_request parameter
                $result['remote_request'] = $request["remote_request"];
            }
        }
    ...
    private function update_onlineStatus($userId) {
        ...
        try {
            ...
            $stmt = $this->booking->prepare("UPDATE users SET online='1', last_seen = now() WHERE user_guid = ? LIMIT 1");
    ...
    private function logRequest(array $default_params, $code = null) {
		...
		try {
			...
			$stmt->execute([
				...
				"{$this->bookingClass->platform} {$this->bookingClass->browser}",
